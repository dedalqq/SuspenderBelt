...
class MySql {
    ...
    function db_insert($table_name, $data = array()) {
        ...
        foreach ($data as $data_id => $data_value) {
            $data_value = self::strHendler($data_value);
            $sql_data_id .= "`" . $data_id . "`,";
    ...
    function db_exec($standart = false, $reset = true) {
        ...
        return $this->db_resulte($result, $standart);
    }
    ...
    private function db_resulte($result, $standart = false) {
        $return_data = array();
        if (mysql_num_rows($result) == 1 && !$standart) {
            for ($j = 0; $j < mysql_num_fields($result); $j++) {
                $return_data[mysql_field_name($result, $j)] = mysql_result($result, 0, mysql_field_name($result, $j));
            }
        }
        else {
    ...
    static public function strHendler($data) {
